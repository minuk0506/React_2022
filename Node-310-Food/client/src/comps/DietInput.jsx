...
const DietInput = ({ params }) => {
  ...
  const onClick = async (e) => {
    ...
    서로 다른 PORT 를 통해서 데이터를 주고 받게 되어
    CORS(Cross Origin Resource share, 교차사이트 스크립트 오류)가 발생을 한다.
    서버와 서버간의 통신(데이터 교환)이 이루어 질때
    보안, 해킹 방지 등을 이유로 점점 많은 곳에서 스크립트 교환이
    이루어지지 않도록 하고 있다.
    CORS 때문에 실제 필요한 API 이용에 제한이 많다.
    React, NodeJS 가 같은 서버에서 작동될 때는 Package.json 에
